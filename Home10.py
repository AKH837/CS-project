...
if selected == "Classes":
    ...
    option = st.selectbox(
        'Select Subject',
        ('Science', 'Math', 'Social-Studies', 'Language'), index=None)
    ...
    if option == 'Science':
        def show_vid(x):
            ...
            def fetch_video(Chapter_no):
                ...
                query = "SELECT video_data FROM SCIENCE WHERE Chapter_no = %s"
    ...
    if option == 'Social-Studies':
        def show_vid(x):
            ...
            def fetch_video(Chapter_no):
                ...
                cursor = conn.cursor()
                # Assuming there's a column 'chapter_number' in your 'videos' table
                query = "SELECT video_data FROM SST WHERE Chapter_no = %s"
                cursor.execute(query, (Chapter_no,))
                result = cursor.fetchone()
                cursor.close()
                conn.close()
    ...
    if option == 'Language':
        def show_vid(x):
            ...
            def fetch_video(Chapter_no):
                ...
                cursor = conn.cursor()
                # Assuming there's a column 'chapter_number' in your 'videos' table
                query = "SELECT video_data FROM LANG WHERE Chapter_no = %s"
                cursor.execute(query, (Chapter_no,))
                result = cursor.fetchone()
                cursor.close()
                conn.close()
